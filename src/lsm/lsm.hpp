...
class LSM {
 ...
 public:
  ...
  LSM<K, V>(unsigned long eltsPerRun, unsigned int numRuns, double merged_frac,
            double bf_fp, unsigned int pageSize, unsigned int diskRunsPerLevel)
      : _eltsPerRun(eltsPerRun),
        _num_runs(numRuns),
        _frac_runs_merged(merged_frac),
        _diskRunsPerLevel(diskRunsPerLevel),
        _num_to_merge(ceil(_frac_runs_merged * _num_runs)),
        _pageSize(pageSize) {
    _activeRun = 0;
    ...
    DiskLevel<K, V> *diskLevel = new DiskLevel<K, V>(
        pageSize, 1, _num_to_merge * _eltsPerRun, _diskRunsPerLevel,
    ...
    for (int i = 0; i < _num_runs; i++) {
      ...
      BloomFilter<K> *bf =
          new BloomFilter<K>(_eltsPerRun, _bfFalsePositiveRate);
  ...
  bool lookup(K &key, V &value) {
    ...
    for (int i = _activeRun; i >= 0; --i) {
      if (key < C_0[i]->get_min() || key > C_0[i]->get_max() ||
          !filters[i]->mayContain(&key, sizeof(K)))
        continue;
  ...
  vector<KVPair<K, V>> range(K &key1, K &key2) {
    ...
    for (int i = _activeRun; i >= 0; --i) {
      ...
      if (cur_elts.size() != 0) {
        eltsInRange.reserve(eltsInRange.size() +
                            cur_elts.size());  // this over-reserves to be safe
  ...
  void printStats() {
    ...
    cout << "Number of Elements in Buffer (including deletes): " << num_buffer()
         << endl;

    for (int i = 0; i < diskLevels.size(); ++i) {
      cout << "Number of Elements in Disk Level " << i
           << "(including deletes): " << diskLevels[i]->num_elements() << endl;
    }
  ...
  void mergeRunsToLevel(int level) {
    ...
    if (level == _numDiskLevels) {  // if this is the last level
      DiskLevel<K, V> *newLevel = new DiskLevel<K, V>(
          ...
          _diskRunsPerLevel, ceil(_diskRunsPerLevel * _frac_runs_merged),
          _bfFalsePositiveRate, read_fd, write_fd);
    ...
    vector<DiskRun<K, V> *> runsToMerge =
    ...
    vector<FileInfo> inputFileInfo;

    for (auto r : runsToMerge) {
      inputFileInfo.push_back(getFileInfo(r->getFName().c_str(), r->getFSize()));
    }

    string ofname = diskLevels[level]->runs[diskLevels[level]->_activeRun]->getFName();
    size_t ofsize = diskLevels[level]->runs[diskLevels[level]->_activeRun]->getFSize();
    FileInfo outputFileInfo = getFileInfo(ofname.c_str(), ofsize);


    // 3. call custom addRuns function.

    //        diskLevels[level]->addRuns(runsToMerge, runLen, isLast);
    int j = diskLevels[level]->addRunsCompute(inputFileInfo, outputFileInfo, isLast);
  ...
  void merge_runs(vector<Run<K, V> *> runs_to_merge,
                  vector<BloomFilter<K> *> bf_to_merge) {
  ...
  void do_merge() {
    ...
    //        mergeThread = thread (&LSM::merge_runs, this,
    //        runs_to_merge,bf_to_merge); // comment for single threaded merging
    merge_runs(runs_to_merge,
               bf_to_merge);  // uncomment for single threaded merging
  ...
  unsigned long num_buffer() {
    ...
    for (int i = 0; i <= _activeRun; ++i) total += C_0[i]->num_elements();
    return total;
  }
  unsigned long size() {
    K min = INT_MIN;
